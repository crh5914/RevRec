...
def parse_args():
    ...
    parser.add_argument('--embedding_dim', type=int, default=50, help="Dimensionality of character embedding")
    parser.add_argument('--filter_sizes', type=str, default='[2]', help='filter size')
    parser.add_argument("--num_filters", type=int,default=50,help="Number of filters per filter size")
...
def generate_batch(data,user_text,item_text,batch_size):
    ...
    for batch_num in range(ll):
        ...
        for idx in range(start_index,end_index):
            u,i,r = data[idx]
            # skip cold start user and item
            if u not in user_text or i not in item_text:
                continue
            uid.append([u])
            iid.append([i])
            y_batch.append([r])
