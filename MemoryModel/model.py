...
class Corpus:
    def __init__(self,dataset,threshold=0.8,filter_stop=True):
    ...
    def load(self):
        ...
        self.num_items = 0
        self.docs = []
        with open('../data/{}/train_{}.txt'.format(self.dataset,self.dataset),'r') as fp:
            for line in fp:
                vals = line.strip().split('\t')
                #print(vals)
                u,i,r = int(vals[0]),int(vals[1]),float(vals[2])
                self.num_users = max(u,self.num_users)
                self.num_items = max(i,self.num_items)
                nline = [u,i,r]
                if len(vals) >= 4:
                    words = vals[3].strip().split(' ')
                    self.docs.append(words)
                self.train_data.append(nline)
        with open('../data/{}/valid_{}.txt'.format(self.dataset,self.dataset),'r') as fp:
           for line in fp:
               vals = line.strip().split('\t')
               u,i,r = int(vals[0]),int(vals[1]),float(vals[2])
               self.num_users = max(u,self.num_users)
               self.num_items = max(i,self.num_items)
               nline = [u,i,r]
               self.valid_data.append(nline)
        with open('../data/{}/test_{}.txt'.format(self.dataset,self.dataset),'r') as fp:
            for line in fp:
                vals = line.strip().split('\t')
                u,i,r = int(vals[0]),int(vals[1]),float(vals[2])
                self.num_users = max(u,self.num_users)
    ...
    def generate_valid_batch(self,batch_size):
        for i in range(0,len(self.valid_data),batch_size):
            end = i + batch_size
            if end > len(self.valid_data):
                end = len(self.valid_data)
            batch_users,batch_items,batch_ratings = [],[],[]
            for j in range(i,end):
                u,i,r = self.valid_data[j]
                batch_users.append(u-1)
                batch_items.append(i-1)
                batch_ratings.append(r)
            yield batch_users,batch_items,batch_ratings    
class SentimentCNN:
    ...
    def train(self,epochs,batch_size):
        ...
        for epoch in range(epochs):
            ...
            vmse,vmae = self.valid(batch_size)
            mse,mae = self.test(batch_size)
    ...
    def valid(self,batch_size):
        ys_,ys = [],[]
        for batch_users,batch_items,batch_ratings in tqdm(self.ds.generate_valid_batch(batch_size)):
            feed_dict = {self.user:batch_users,self.item:batch_items,self.rating:batch_ratings,self.dropout_keep_prob:1.0,self.alpha:0.1}
            y_ = self.sess.run(self.y_,feed_dict=feed_dict)
            ys_ += list(y_)
            ys += batch_ratings
